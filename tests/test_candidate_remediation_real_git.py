...
def test_real_git_restart_after_workspace_ready_preserves_execution_boundary(
    ...
    remediation = CandidateRemediation(
        ...
        status=RemediationStatus.WORKSPACE_READY,
...
def test_real_git_restart_during_implementer_running_refuses_duplicate_invocation(
    ...
    remediation = CandidateRemediation(
        ...
        contract_hash=contract.contract_hash(),
        contract_payload=contract.canonical_payload(),
        status=RemediationStatus.IMPLEMENTER_RUNNING,
        workspace_path=str(workspace.path),
        branch_name=workspace.branch_name,
        authorized_paths=contract.allowed_paths,
    )
...
def test_real_git_restart_after_implementer_completed_reuses_authorized_changes(
    ...
    remediation = CandidateRemediation(
        ...
        contract_hash=contract.contract_hash(),
        contract_payload=contract.canonical_payload(),
        status=RemediationStatus.IMPLEMENTER_COMPLETED,
        workspace_path=str(workspace.path),
        branch_name=workspace.branch_name,
        authorized_paths=contract.allowed_paths,
    )
...
def test_real_git_restart_after_scope_validated_finalizes_once(tmp_path, in_memory_uow):
    ...
    remediation = CandidateRemediation(
        ...
        contract_hash=contract.contract_hash(),
        contract_payload=contract.canonical_payload(),
        status=RemediationStatus.SCOPE_VALIDATED,
        workspace_path=str(workspace.path),
        branch_name=workspace.branch_name,
        authorized_paths=contract.allowed_paths,
        tree_fingerprint=asyncio.run(manager.working_state_fingerprint(workspace.path)),
    )
    in_memory_uow.candidate_remediations.save(remediation)
    implementer = FakeImplementer()
    service = CandidateRemediationService(
...
def test_real_git_restart_after_candidate_persisted_reuses_exact_result(
    ...
    result_sha = asyncio.run(
        manager.finalize_candidate_commit(
            workspace.path, job.job_id, "p", remediation_id, contract.contract_hash()
        )
    )
    result_manifest = CandidateManifestService().generate_manifest(
        workspace.path, result_sha, job.job_id
    )
    in_memory_uow.candidate_manifests.save(result_manifest)
    result_candidate = OrchestrationCandidate(
        run_id="run",
        generation=2,
        base_sha=source_sha,
        candidate_sha=result_sha,
        candidate_ref=workspace.branch_name,
        manifest_id=result_manifest.manifest_id,
        manifest_hash=result_manifest.manifest_hash,
        authorship_summary={"remediation_id": remediation_id},
        is_frozen=True,
    )
    in_memory_uow.orchestration_candidates.save(result_candidate)
    in_memory_uow.orchestration_candidates.supersede(
        source.candidate_id, result_candidate.candidate_id
    )
    ...
    remediation = CandidateRemediation(
        ...
        result_generation=2,
        result_candidate_sha=result_sha,
    )
    in_memory_uow.candidate_remediations.save(remediation)
    restarted_runner = FakeImplementer()
    restarted = CandidateRemediationService(
        in_memory_uow,
        tmp_path,
        pipeline=SimpleNamespace(implementer_runner=restarted_runner, checks_runner=ChecksRunner()),
        worktree_manager=manager,
    )
...
def test_real_execution_pipeline_routes_remediation_to_its_implementer_runner(
    ...
    manager = WorktreeManager(tmp_path, uow=in_memory_uow)
    service = CandidateRemediationService(
        in_memory_uow, tmp_path, pipeline=pipeline, worktree_manager=manager
    )
    contract = RemediationContract(
        contract_version="1",
        run_id="run",
        ...
        change_name="change",
        objective="fix",
        allowed_paths=["src/pipeline-fix.py"],
    )
    result = service.remediate("run", contract)
    assert result.status == RemediationStatus.COMPLETED
...
def test_real_git_failed_n_plus_one_then_explicit_n_plus_two_preserves_lineage(
    ...
    n_plus_one_identity = (
        n_plus_one.candidate_sha,
        n_plus_one.candidate_ref,
        n_plus_one.manifest_id,
        n_plus_one.manifest_hash,
        n_plus_one.base_sha,
    )
    project.checks = [{"name": "pass", "command": "true"}]
    in_memory_uow.projects.save(project)
    contract2 = RemediationContract(
        contract_version="1",
        run_id="run",
        source_candidate_generation=2,
        source_candidate_sha=n_plus_one.candidate_sha,
        source_candidate_base_sha=n_plus_one.base_sha,
        change_name="change",
        objective="fix two",
        allowed_paths=["src/fix2.py"],
    )
    ...
    assert (
        ...
        source.base_sha,
    ) == (source_sha, "refs/heads/main", manifest.manifest_id, manifest.manifest_hash, source_sha)
    assert (
        n_plus_one.candidate_sha,
        n_plus_one.candidate_ref,
        n_plus_one.manifest_id,
