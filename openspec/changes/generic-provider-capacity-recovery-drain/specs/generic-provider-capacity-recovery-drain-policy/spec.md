# Specification: Generic Provider Capacity Recovery & Drain Policy

## Requirements

### REQ-GENERIC-PROVIDER-CAPACITY-RECOVERY-DRAIN-POLICY-1: Primary Capability
...
#### Scenario 4: Scheduler contains no Codex-specific recovery branching
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Scheduler contains no Codex-specific recovery branching.

#### Scenario 5: Scheduler contains no Antigravity-specific recovery branching
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Scheduler contains no Antigravity-specific recovery branching.

#### Scenario 6: Provider-specific recovery behavior is isolated behind provider adapters
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Provider-specific recovery behavior is isolated behind provider adapters.

#### Scenario 7: Codex temporarily_unavailable is periodically re-probed
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Codex temporarily_unavailable is periodically re-probed.

#### Scenario 8: Antigravity temporarily_unavailable is periodically re-probed
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Antigravity temporarily_unavailable is periodically re-probed.

#### Scenario 9: Fake/test provider uses the same generic recovery path
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Fake/test provider uses the same generic recovery path.

#### Scenario 10: Positive probe transitions provider to available automatically
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Positive probe transitions provider to available automatically.

#### Scenario 11: Negative probe preserves temporarily_unavailable with fresh evidence
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Negative probe preserves temporarily_unavailable with fresh evidence.

#### Scenario 12: Provider probes create 0 Runs
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Provider probes create 0 Runs.

#### Scenario 13: Provider probes create 0 Jobs
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Provider probes create 0 Jobs.

#### Scenario 14: Provider probes consume 0 implementation retry budget
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Provider probes consume 0 implementation retry budget.

#### Scenario 15: Known expected reset is persisted and visible
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Known expected reset is persisted and visible.

#### Scenario 16: AUTO_DISCOVERED reset source is supported
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** AUTO_DISCOVERED reset source is supported.

#### Scenario 17: OPERATOR_REPORTED reset source is supported
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** OPERATOR_REPORTED reset source is supported.

#### Scenario 18: Operator can enter or update expected provider return time
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Operator can enter or update expected provider return time.

#### Scenario 19: Unknown reset can enter bounded WAITING_CAPACITY
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Unknown reset can enter bounded WAITING_CAPACITY.

#### Scenario 20: Blind automatic wait is bounded to 2 hours
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Blind automatic wait is bounded to 2 hours.

#### Scenario 21: After blind-wait threshold explicit operator decisions are surfaced
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** After blind-wait threshold explicit operator decisions are surfaced.

#### Scenario 22: Keep Waiting does not consume implementation retry budget
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Keep Waiting does not consume implementation retry budget.

#### Scenario 23: Drain provider never starts BACKLOG work
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Drain provider never starts BACKLOG work.

#### Scenario 24: Drain provider never starts READY work
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Drain provider never starts READY work.

#### Scenario 25: Drain provider never starts QUEUED not-yet-started work
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Drain provider never starts QUEUED not-yet-started work.

#### Scenario 26: Drain eligibility requires material execution already started
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Drain eligibility requires material execution already started.

#### Scenario 27: Drain handoff preserves valid existing work and evidence
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Drain handoff preserves valid existing work and evidence.

#### Scenario 28: Drain handoff continues the same Run/Job lifecycle
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Drain handoff continues the same Run/Job lifecycle.

#### Scenario 29: Drain does not create duplicate Run/Job/backlog item
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Drain does not create duplicate Run/Job/backlog item.

#### Scenario 30: Original provider recovery can resume waiting work automatically when safe
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Original provider recovery can resume waiting work automatically when safe.

#### Scenario 31: PWA exposes provider status, reason, wait timing, probes, reset and drain eligibility
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** PWA exposes provider status, reason, wait timing, probes, reset and drain eligibility.

#### Scenario 32: TUI exposes equivalent provider-capacity state
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** TUI exposes equivalent provider-capacity state.

#### Scenario 33: PWA, TUI and PostgreSQL agree
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** PWA, TUI and PostgreSQL agree.

#### Scenario 34: Provider recovery and operator decisions are auditable
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Provider recovery and operator decisions are auditable.

#### Scenario 35: Current Codex behavior works through generic provider contract
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Current Codex behavior works through generic provider contract.

#### Scenario 36: Current Antigravity behavior works through generic provider contract
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Current Antigravity behavior works through generic provider contract.

#### Scenario 37: Future provider replacement requires adapter implementation, not scheduler rewrite
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Future provider replacement requires adapter implementation, not scheduler rewrite.

#### Scenario 38: Real production validation demonstrates recovery without human provider-state repair
- **GIVEN** the configured environment for `mini me`,
- **WHEN** the capability is invoked,
- **THEN** Real production validation demonstrates recovery without human provider-state repair.
