...
- CORRECTION (Finding 5): the above reviewer proving used `--mode accept-edits` (the implementer profile) rather than the configured reviewer profile `--mode plan`. It is preserved as valid historical execution evidence (an edit-capable invocation happened not to mutate), but it did not prove the configured reviewer profile.
- CORRECTED AG reviewer behavioral proving EXECUTED 2026-09-16T21:56:14Z–21:57:30Z (agy 1.2.4, reviewer model: gpt-oss-120b-medium, distinct from implementer model gemini-3.6-flash-medium, `--mode plan --dangerously-skip-permissions --print-timeout 1h`, candidate SHA bfcf230489321f2688486a133e7b1a803e4ac6b7). Result: PASS — verdict ACCEPTABLE, HEAD before == HEAD after == bfcf230489321f2688486a133e7b1a803e4ac6b7, worktree clean before/after, no unstaged/staged/untracked mutation, no reviewer commit. Evidence durable at docs/evidence/021_provider_execution_safety_stabilization_ag_reviewer_configured_profile_proving.md (raw output + prompt at docs/evidence/021_provider_execution_safety_ag_proving/).
