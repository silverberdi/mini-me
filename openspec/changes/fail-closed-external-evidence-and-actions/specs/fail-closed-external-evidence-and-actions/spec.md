...
The system SHALL represent all external adapter queries and mutating side effects using a strongly typed container (`ExternalActionResult[T]`) with explicit outcome states (`SUCCESS`, `FAILURE`, `UNKNOWN`, `AMBIGUOUS`), explicit fail-closed retry safety (`RetrySafety.SAFE`, `UNSAFE`, `UNKNOWN`), required source adapter identity, required typed reason code, and observed evidence metadata.

#### Scenario: Successful external operation returns SUCCESS with observed data and SAFE retry
GIVEN a valid external REST, CLI, or Git operation
WHEN the remote system returns an authoritative positive response
THEN the adapter SHALL return outcome SUCCESS
AND source_adapter and reason_code SHALL be explicitly populated
AND retry_safety SHALL be marked SAFE
...
AND retry_safety SHALL be marked SAFE (read-only query is idempotent).
...
### Requirement: Elimination of fabricated default success and default parameters
...
### Requirement: Stable operation identity and marker-based deduplication
...
THEN issue reuse (SUCCESS with reason_code REUSED_EXISTING) SHALL be authorized ONLY if an existing issue body contains the exact marker comment `<!-- minime-opkey: K -->`
...
THEN GitHubAdapter SHALL return outcome SUCCESS with reason_code ALREADY_CLOSED and retry_safety SAFE
...
THEN GitHubAdapter SHALL return outcome SUCCESS with data set to existing PR details and reason_code REUSED_EXISTING
...
THEN GitHubAdapter SHALL return outcome SUCCESS with reason_code ALREADY_ABSENT and retry_safety SAFE
...
THEN deployment result SHALL report deploy_action_outcome = SUCCESS with reason_code EXECUTION_SUCCESS
