...
Provides an interactive terminal operator console (`minime console` / `minime tui`) built with Textual that presents multi-project system status, attention items, active executions, pipeline progression, candidate lineage, check diagnostics, review findings, DeepSeek audit risk, 013 preview state, and transition history without direct PostgreSQL queries from UI widgets.
