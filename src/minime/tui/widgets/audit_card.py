"""DeepSeek audit summary card widget."""

from __future__ import annotations

from rich.text import Text
from textual.app import ComposeResult
from textual.containers import Vertical
from textual.reactive import reactive
from textual.widget import Widget
from textual.widgets import Static

from minime.services.dashboard_service import AuditSummaryDTO
from minime.tui.models import get_risk_text, get_status_text, sanitize_text, short_sha


class AuditSummaryWidget(Widget):
    """Widget displaying DeepSeek audit risk level, material findings, and candidate binding."""

    DEFAULT_CSS = """
    AuditSummaryWidget {
        background: #1e293b;
        border: round #334155;
        padding: 1;
        margin-bottom: 1;
        height: auto;
    }
    """

    audit: reactive[AuditSummaryDTO | None] = reactive(None)

    def compose(self) -> ComposeResult:
        with Vertical():
            yield Static("DEEPSEEK INDEPENDENT AUDIT", classes="panel-title")
            yield Static("No audit recorded", id="audit-content")

    def watch_audit(self, val: AuditSummaryDTO | None) -> None:
        content = self.query_one("#audit-content", Static)
        if val is None or val.status == "not_started":
            content.update(
                Text("Audit not yet performed for current candidate.", style="dim italic")
            )
            return

        body = Text()

        # Risk & Status
        risk_badge = get_risk_text(val.risk)
        status_badge = get_status_text(val.status)
        body.append("Audit Status: ")
        body.append_text(status_badge)
        body.append("  Risk: ")
        body.append_text(risk_badge)
        body.append("\n")

        # Provider & Candidate Binding
        body.append(f"Provider: {val.provider or 'DeepSeek Direct'}")
        if val.model:
            body.append(f" ({val.model})")
        body.append("\n")

        if val.is_stale_to_current_candidate:
            body.append("⚠ STALE AUDIT: Bound to older candidate generation (", style="bold yellow")
            body.append(f"{short_sha(val.candidate_sha)})\n", style="yellow")
        else:
            body.append(f"Bound to candidate SHA: {short_sha(val.candidate_sha)}\n", style="dim")

        # Findings Summary
        if val.material_findings_count > 0:
            body.append(f"Material Findings: {val.material_findings_count}\n", style="bold red")
        else:
            body.append("Material Findings: 0 (Passed)\n", style="green")

        if val.summary:
            body.append(f"\nSummary: {sanitize_text(val.summary)}\n", style="white")

        if val.findings:
            body.append("\nAudit Findings Details:\n", style="bold white")
            for f in val.findings[:3]:
                desc = sanitize_text(f.get("description", str(f)))
                sev = f.get("severity", "finding").upper()
                body.append(f"  • [{sev}] {desc}\n", style="yellow")

        content.update(body)
